...
struct LocationSelectingViewWrapper: View {
    @State private var key = UUID() // Unique key to reset the view
    
    var body: some View {
        LocationSelectingView(resetKey: $key)
            .id(key) // Attach the unique key to the view
            .onChange(of: key) { _ in
                print("LocationSelectingView recreated")
            }
    }
}
...
struct LocationSelectingView: View {
    ...
    @Environment(\.colorScheme) var colorScheme
    @StateObject private var viewModel = LocationSelectingViewModel()
    @Binding var resetKey: UUID // Pass the reset key from the wrapper
    ...
    var body: some View {
        if AppConstants.isAuthenticated {
            ...
            .onChange(of: viewModel.navigationPath) { newValue in
                if newValue.isEmpty {
                    viewModel.resetViewsForRideSelection()
                    resetKey = UUID() // Trigger view recreation
                }
            }
            .bottomSheet(
                bottomSheetPosition: $viewModel.bottomSheetPosition,
...
#Preview {
    LocationSelectingView(resetKey: .constant(UUID()))
}
