...
//
//   let rideBookingResponse = try? JSONDecoder().decode(RideBookingResponse.self, from: jsonData)

import Foundation

// MARK: - RideBookingResponse
struct RideBookingResponse: Codable {
    let status: Int?
    let message: String?
    let data: RideBookingData?
}
...
// MARK: - NAddress
struct NAddress: Codable {
    let lat, long, address: String?
    let addressID: JSONNull?
    let id: String?

    enum CodingKeys: String, CodingKey {
        case lat, long, address
